...
export default function Translate() {
  ...
  return (
    <>
      {/* --- EL DISEÑO Y LA POSICIÓN SE MANTIENEN IGUAL --- */}
      <Fab
        color="primary"
        onClick={handleOpen}
        sx={{
          position: "fixed",
          background: "linear-gradient(to right, #e85252, #b73939)",
          color: "#fff",
          zIndex: 9999,
      ...
      >
        {idiomas.map((idioma) => (
          <MenuItem
            key={idioma.code}
            // "notranslate" ya no es necesario aquí.
            selected={i18n.language === idioma.code}
            onClick={() => cambiarIdioma(idioma.code)}
            sx={{
              // --- TUS ESTILOS PERSONALIZADOS SE CONSERVAN ---
              "&.Mui-selected": {
                background:
                  "linear-gradient(90deg, rgba(232,82,82,1) 0%, rgba(183,57,57,1) 100%)",
                color: "#fff",
              },
              "&.Mui-selected:hover": {
                background:
                  "linear-gradient(90deg, rgba(210,72,72,1) 0%, rgba(160,47,47,1) 100%)",
              },
            }}
          >
            {idioma.label}
